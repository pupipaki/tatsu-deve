# keyword　から Gemini生成して、articleへ記事を移動する
import os
import requests
import json
import random
...
def get_outline_from_gemini(title):
    """Gemini APIを使用してブログ記事を生成"""
    try:
        prompt = f"""
「{title}」というタイトルを検索する人が、今もっとも知りたい「最新の解決策」に焦点を当てたブログ記事のリード文を作成してください。
...
【条件】
-リード文全体はは500字以内とする
-体験談は具体的な数字や状況を含めて赤裸々な感情を描写する
-迷いや葛藤を本音で書く
-読者が明日から実践できる内容を1行で示す
-文末は読者に親しみやすい口調。
- 読者が直面している「最新の課題」を生々しく記載する。
- SEOを反映したブログタイトル案を追加する。
- 最後に、ブログ記事をXで知らせるときの告知文を記載する。
"""
